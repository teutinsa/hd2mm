...
pub struct ModManifest {
    ...
    data: ModData,
    #[serde(skip)]
    translated_options: OnceCell<[ModOption; 1]>,
}
...
impl ModManifest {
    ...
    pub fn new(
        ...
        Self {
            ...
            translated_options: OnceCell::new(),
        }
    }
    ...
    pub fn options(&self) -> Option<&[ModOption]> {
        match &self.data {
            ModData::Legacy { options, .. } => options.as_ref().map(|v| {
                debug!("Obtaining translated options");
                self.translated_options
                    .get_or_init(|| {
                        debug!("Translating options");
                        let opts: Vec<ModSubOption> = v
                            .iter()
                            .map(|s| {
                                ModSubOption::new(
                                    s.to_owned(),
...
mod test {
    ...
    fn mod_manifest_deserialize_legacy() {
        let data = r#"
		{
            "Guid": "010000aa-0000-4d2e-a838-2a78324a6ccb",
            "Name": "Helldivas - Liberty's Ladies Complete Base Pack",
            "Description": "H-LL 010000 CBP v1.1.5 / / / H-LL model replacements for all available armor sets",
            "IconPath": "H-LL_Icon1_APack.png",
            "Options": [
                "Base"
            ]
        }
